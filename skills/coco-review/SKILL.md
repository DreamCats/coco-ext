...
用于生成和查看代码审查报告。该能力已经接入 `pre-push` 自动触发，但 skill 仍然负责说明何时手动补跑、如何查看产物、以及异常时如何定位。
...
- 执行 `git push` 时，`pre-push` 会异步触发 `coco-ext review --async`
- 自动触发只负责后台启动，不保证报告在 push 返回前生成完成
...
- 如果 push 已经自动触发 review，除非用户明确要求，否则不要重复再跑一次同步 review
