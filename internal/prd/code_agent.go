...
// 不塞源文件内容，让 agent 自己读。只告诉 agent 做什么 + 在哪做。
func BuildAgentCodePrompt(taskDir, workDir string) string {
	var b strings.Builder

	b.WriteString("你是一个代码实现 agent。请基于需求文档，在当前仓库中完成所有代码改动。\n\n")

	b.WriteString("## 工作目录\n\n")
	b.WriteString(fmt.Sprintf("当前仓库根目录: %s\n\n", workDir))

	b.WriteString("## 输入文档\n\n")
	b.WriteString(fmt.Sprintf("1. 技术方案: %s\n", filepath.Join(taskDir, "design.md")))
	b.WriteString(fmt.Sprintf("2. 实施计划: %s\n", filepath.Join(taskDir, "plan.md")))
...
func GenerateCodeWithAgent(agent *generator.AgentGenerator, taskDir, workDir string, now time.Time, onChunk func(string), onTool func(generator.ToolEvent)) (*AgentCodeResult, error) {
	prompt := BuildAgentCodePrompt(taskDir, workDir)
