...
用于管理 `coco-ext prd` 工作流。目标不是替代人工判断需求是否合理，而是把“什么时候用 `run`、什么时候拆成 `refine -> plan -> code`、失败后先看哪里”固定下来，避免每次重新摸索。
...
5. 如果结果不满意，执行 `prd reset` 清理 worktree 和分支，再重新 `prd code`。
6. 结果确认完成后，执行 `prd archive` 收尾。
...
- `.livecoding/tasks/<task-id>/source.json`
- `.livecoding/tasks/<task-id>/prd.source.md`
- `.livecoding/tasks/<task-id>/prd-refined.md`
- `.livecoding/tasks/<task-id>/design.md`
- `.livecoding/tasks/<task-id>/plan.md`
- `.livecoding/tasks/<task-id>/code-result.json`
...
- `prd reset` / `prd archive` 会删除对应 worktree 和分支
