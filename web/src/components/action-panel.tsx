...
export function ActionPanel({ task }: { task: TaskRecord }) {
  ...
  return (
    <section className="rounded-[24px] border border-stone-200 bg-white p-4">
      <div className="mb-4 text-xs font-semibold uppercase tracking-[0.22em] text-stone-500">Suggested Commands</div>
      <div className="space-y-3">
        {commands.map((command, index) => (
          <div className="rounded-[18px] border border-stone-200 bg-stone-50 px-3 py-3" key={`${command}-${index}`}>
            <div className="mb-2 text-[11px] uppercase tracking-[0.2em] text-stone-500">
              {index === 0 ? 'Primary Next' : `Repo Next ${index}`}
            </div>
            <div className="rounded-2xl border border-stone-200 bg-white px-3 py-3 font-mono text-xs leading-6 text-stone-800">
              {command}
            </div>
          </div>
        ))}
