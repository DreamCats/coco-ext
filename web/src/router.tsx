  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_rgba(34,197,94,0.12),_transparent_32%),radial-gradient(circle_at_top_right,_rgba(249,115,22,0.15),_transparent_28%),linear-gradient(180deg,_#f6f4ee_0%,_#f2efe7_52%,_#ede7dc_100%)] text-stone-950">
      <div className="mx-auto flex min-h-screen max-w-[1600px] flex-col px-4 py-4 md:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-4xl">
                <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.24em] text-emerald-700">
                  AI Delivery Workbench
                </div>
                <h1 className="text-[42px] font-semibold tracking-[-0.06em] text-stone-950 md:text-[52px]">
                  From PRD To Code
                </h1>
            ...
            <div className="grid gap-4 border-t border-stone-200/80 pt-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(360px,0.85fr)]">
              ...
              <div className="grid gap-4">
                <section className="rounded-[24px] border border-stone-200 bg-stone-50/80 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-stone-500">Repo Orchestration</div>
                      <h3 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-stone-950">多仓任务不是附属能力</h3>
                    </div>
                    <div className="rounded-full border border-stone-200 bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-stone-500">
                      top repos
                    </div>
                  </div>
                  <div className="mt-4 space-y-3">
                    {repoSignals.length > 0 ? (
                      repoSignals.map((repo) => (
                        <div
                          className="grid gap-3 rounded-[18px] border border-stone-200 bg-white px-4 py-3 sm:grid-cols-[minmax(0,1fr)_120px_120px]"
                          key={repo.repoId}
                        >
                          <div>
                            <div className="text-sm font-semibold text-stone-950">{repo.repoId}</div>
                            <div className="mt-1 text-xs text-stone-500">作为 task 作用范围反复出现的 repo</div>
                          </div>
                          <div>
                            <div className="text-[11px] uppercase tracking-[0.2em] text-stone-500">tasks</div>
                            <div className="mt-2 text-lg font-semibold text-stone-950">{repo.taskCount}</div>
                          </div>
                          <div>
                            <div className="text-[11px] uppercase tracking-[0.2em] text-stone-500">in flight</div>
                            <div className="mt-2 text-lg font-semibold text-stone-950">{repo.activeCount}</div>
                          </div>
                        </div>
                      ))
                    ) : (
                      <div className="rounded-[18px] border border-dashed border-stone-300 bg-white px-4 py-4 text-sm text-stone-500">
                        当前还没有 repo scope 数据。
                      </div>
                    )}
                  </div>
            ...
            <div className="flex flex-wrap items-center gap-3 border-t border-stone-200/80 pt-4 text-xs uppercase tracking-[0.2em] text-stone-500">
              <span>Input</span>
              <span>→</span>
              <span>Refine</span>
              <span>→</span>
        ...
        <main className="relative min-h-0 flex-1 overflow-y-auto rounded-[32px] border border-stone-200/80 bg-white/72 p-4 shadow-[0_1px_0_rgba(255,255,255,0.7)_inset,0_40px_80px_rgba(64,45,18,0.08)] backdrop-blur lg:p-5">
          <Outlet />
        </main>
      </div>
    </div>
